...
void DetectRoom() {
  readLED();
  if (bl < rl && br < rr && bl < gl && br < gr && gl > rl && gr > rr && bl >= 600 & br >= 600) while(1) walk(-200,-200); delay(50);
}
...
void touch(int lim_p) 
{
  ...
  else
  {
    ...
    while ( digitalRead(TOUCH1) || digitalRead(TOUCH2) )// Until the two sensors touch the wall
    {
        ...
        walk(-500,-500); // while the back buttons dont touch the wall make the robot go back
        if(!digitalRead(TOUCH1))walk(0,-500); // while the left back button doesn´t touch the wall walk with the left motor 
        if(!digitalRead(TOUCH2))walk(-500,0); // while the right back button doesn´t touch the wall walk with the right motor
        delay(25);
...
void go_to_middle() {
  ...
  //if the values of left or right are around 30, the entrance was in the middle
    ...
    //if there's exit in the middle 
      ...
      dists(0);
      val_top = 0.0;
      while (val_top <= (((float)size/2.0) - 3.0)) {
        val_top=0.0;
        for(int i = 0; i < 3; i++){
          float med = 0.0;
          med = getUltra(3);
