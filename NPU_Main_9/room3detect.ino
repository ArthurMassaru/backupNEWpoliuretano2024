...
void touch(int lim_p) 
{
  ...
  while ( digitalRead(TOUCH1) || digitalRead(TOUCH2) )// Until the two sensors touch the wall
  {
      ...
      if(!digitalRead(TOUCH1))walk(0,SWR); // while the left back button doesn´t touch the wall walk with the left motor 
      if(!digitalRead(TOUCH2))walk(-SWL,0); // while the right back button doesn´t touch the wall walk with the right motor
      delay(25);
